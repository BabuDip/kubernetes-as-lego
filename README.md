...
humans) and `order_number` (a `PositiveIntegerField` backed by a real PostgreSQL
sequence, so concurrent checkouts can never collide), displayed as `display_id`
(e.g. `PO-0042`) — that's the number staff read out loud and customers see.
